...
async def request_email_confirmation(
    email: EmailStr, db_session: AsyncSession = Depends(get_session)
):
...
async def confirm_email(
    code: str, db_session: AsyncSession = Depends(get_session)
):
    ...
    if not confirmation_code or confirmation_code.code != code:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail='Неправильный код'
        )
...
async def request_password_restoration_code(
    ...
    kafka_message = SendEmailScheme(
        ...
        params={
            ...
            f'{front_config.change_password_endpoint}'
