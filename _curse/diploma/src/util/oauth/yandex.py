...
from src.settings import YandexOauth2Config
...
class YandexOAuth2Authorize(BaseOauth2Authorize):
    def __init__(
            self,
            config: YandexOauth2Config,
    ...
    async def callback(
        ...
        if response.status_code != 200 or 'error' in response.json():
            raise HTTPException(
                ...
                detail='Неправильный код аутентификации'
            )
